"""class Plane:
    ...
    #classmethod
    @classmethod
    def set_extend(cls, amount):
        cls.extend = amount

    @classmethod
    def from_string(cls, aircraft_str):
        name, range = aircraft_str.split('-')
        return cls(name, range)

    #method
    def command(self,x):
            if x == self.name:
                self.flight(1000 * self.extend)
            elif x == "Stay Grounded":
                self.state = "I am Grounded"
            else:
                self.state = "Under Maintenance"

    def flight(self,dist):
        print(self.name + " flies " + str(dist) + " miles.")

class Narrowbody(Plane):
    extend = 1.5

    def __init__(self,name,range,capacity):
        super().__init__(name,range)
        self.capacity = capacity

class Boeing(Plane):

    def __init__(self,name,range,product=None):
        super().__init__(name,range)
        if product is None:
            self.product = []
        else:
...
A320 = Plane("A320","short-haul")
B737 = Plane("B737","short-haul")
A350 = Plane("A350","inter-continental")
A321 = Plane("A321","trans-atlantic")

print(A320.range)
print(B737.range)

A320.command("Stay Grounded")
print("[A320 Status]: " + A320.state)
B737.command("Under Maintenance")
print("[B737 Status]: " + B737.state)
Plane.set_extend(1.8)
A320.command("A320")
print(Plane.fleet)
A320.flight(10000)

A319 = Narrowbody("A319","regional","100")
print(A319.capacity)
